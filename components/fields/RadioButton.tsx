...
function DesignerComponent({
  ...
  return (
    <div className='flex flex-col gap-2 w-full'>
      ...
      {helperText && (
        <p className='text-muted-forground text-[0.8rem]'>{helperText}</p>
      )}
    </div>
  );
}
function FormComponent({
  ...
  return (
    <div className='flex flex-col gap-2 w-full'>
      ...
      <RadioGroup
        className={cn("w-full flex flex-wrap", error && "border-red-500")}
        defaultValue={value}
        onValueChange={(value) => {
          setValue(value);
          if (!submitValue) return;
          const valid = RadioButtonFieldFormElement.validate(element, value);
          setError(!valid);
      ...
      {helperText && (
        <p
          className={cn(
            "text-muted-forground text-[0.8rem]",
            error && "text-red-500"
          )}
        >
          {helperText}
